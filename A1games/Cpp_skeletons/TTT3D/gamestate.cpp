...
std::string GameState::toString(int pPlayer) const
{
	...
	ss << "      layer 0            layer 1            layer 2            layer 3 \n";
	ss << board_top;
	ss << "  0 " << board_left;
	for (int c = 0; c < 4; ++c)
		ss << cell_text[at(0, c, 0)];
	ss << board_right << " 3  "
	   << " 16 " << board_left;
	for (int c = 0; c < 4; ++c)
		ss << cell_text[at(0, c, 1)];
	ss << board_right << " 19 "
	   << " 32 " << board_left;
	for (int c = 0; c < 4; ++c)
		ss << cell_text[at(0, c, 2)];
	ss << board_right << " 35 "
	   << " 48 " << board_left;
	for (int c = 0; c < 4; ++c)
		ss << cell_text[at(0, c, 3)];
	ss << board_right << " 51  Last move: " + mLastMove.toString() + (is_winner ? " (WOHO! I WON!)\n" : "\n");
	ss << "  4 " << board_left;
	for (int c = 0; c < 4; ++c)
		ss << cell_text[at(1, c, 0)];
	ss << board_right << " 7  "
	   << " 20 " << board_left;
	for (int c = 0; c < 4; ++c)
		ss << cell_text[at(1, c, 1)];
	ss << board_right << " 23 "
	   << " 36 " << board_left;
	for (int c = 0; c < 4; ++c)
		ss << cell_text[at(1, c, 2)];
	ss << board_right << " 39 "
	   << " 52 " << board_left;
	for (int c = 0; c < 4; ++c)
		ss << cell_text[at(1, c, 3)];
	ss << board_right << " 55  Next player: " + cell_text[mNextPlayer] + (is_my_turn ? " (My turn)\n" : " (Opponents turn)\n");
	ss << "  8 " << board_left;
	for (int c = 0; c < 4; ++c)
		ss << cell_text[at(2, c, 0)];
	ss << board_right << " 11 "
	   << " 24 " << board_left;
	for (int c = 0; c < 4; ++c)
		ss << cell_text[at(2, c, 1)];
	ss << board_right << " 27 "
	   << " 40 " << board_left;
	for (int c = 0; c < 4; ++c)
		ss << cell_text[at(2, c, 2)];
	ss << board_right << " 43 "
	   << " 56 " << board_left;
	for (int c = 0; c < 4; ++c)
		ss << cell_text[at(2, c, 3)];
	ss << board_right << " 59  X pieces:   " << X_pieces << "\n";
	ss << " 12 " << board_left;
	for (int c = 0; c < 4; ++c)
		ss << cell_text[at(3, c, 0)];
	ss << board_right << " 15 "
	   << " 28 " << board_left;
	for (int c = 0; c < 4; ++c)
		ss << cell_text[at(3, c, 1)];
	ss << board_right << " 31 "
	   << " 44 " << board_left;
	for (int c = 0; c < 4; ++c)
		ss << cell_text[at(3, c, 2)];
	ss << board_right << " 47 "
	   << " 60 " << board_left;
	for (int c = 0; c < 4; ++c)
		ss << cell_text[at(3, c, 3)];
	ss << board_right << " 63  O pieces: " << O_pieces << "\n";
	ss << board_bottom;
...
/*namespace TICTACTOE3D*/ // namespace TICTACTOE3D
}
